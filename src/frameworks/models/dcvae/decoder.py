...
class Decoder(Model):
    def __init__(self, latent_dim, img_size, ch=64, **kwargs):
        name = kwargs.pop('name', 'decoder') 
        self.img_size = img_size
        super(Decoder, self).__init__(**kwargs, name=name)
        small = (img_size[0] == 32)  
        ...
        self.to_rgb_mu = Conv2DTranspose(img_size[-1], (5,5), (2,2), 'SAME',
            activation=tf.math.tanh,
            kernel_initializer=tf.random_normal_initializer(stddev=0.02),
            name='{}_to_rgb_mu'.format(name)) 
    ...
    def call(self, inputs, training=False):
        ...
        rgb_mu = self.to_rgb_mu(net, training=training)
        return rgb_mu